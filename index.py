...
class gdrive_main():
    ...
    def set_auth_url(self,get):
        ...
        with open('/www/server/panel/plugin/gdrive/token.json', 'w') as token:
            json.dump(credentials_data,token)
        public.ExecShell('\cp -rp /www/server/panel/plugin/gdrive/token.json /root/gdrive_token.json')
        public.ExecShell('ln -sf /www/server/panel/plugin/gdrive/token.json /root/gdrive_token.json')
        if not self.set_creds():
            return public.returnMsg(False, "验证失败，请根据页面1 2 3 步骤完成验证")
        return public.returnMsg(True, "验证成功")
